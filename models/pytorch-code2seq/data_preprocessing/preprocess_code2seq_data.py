...
def _vocab_from_counters(config: PreprocessingConfig, token_counter: Counter, target_counter: Counter, type_counter: Counter) -> Vocabulary:
    vocab = Vocabulary()
...
def collect_vocabulary(config: PreprocessingConfig, data_folder: str) -> Vocabulary:
...
def convert_vocabulary(config: PreprocessingConfig, data_folder: str) -> Vocabulary:
...
def preprocess(problem: str, data: str, is_vocab_collected: bool, n_jobs: int, data_folder: str, just_test: bool, test_name: str):
    ...
    if path.exists(vocab_path):
        ...
        vocab = collect_vocabulary(config, data_folder) if is_vocab_collected else convert_vocabulary(config, data_folder)
...
if __name__ == "__main__":
    ...
    preprocess(args.problem, args.data, args.collect_vocabulary, args.n_jobs or cpu_count(), args.data_folder, args.just_test, args.test_name)
