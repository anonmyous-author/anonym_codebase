...
def apply_gradient_attack_v2(data, model, token_to_id, id_to_token, replace_tokens, field_name, opt, orig_tok_map, idx_to_fname, label_to_id, id_to_label, n_samples, device='cpu'):
	...
	def element_wise_pad_flat(tokens, contexts_per_label, tok_map):
		...
		return start_tokens_flat, start_tokens_lens
	...
	for params in itertools.product(li_u_optim_technique, li_u_init_pgd, li_learning_rate, li_use_loss_smoothing, li_use_u_discrete):
		...
		for bid, batch in enumerate(tqdm.tqdm(batch_generator, total=n_samples)):
			...
			print(from_tokens.shape, len(filenames), len(contexts_per_label), labels.shape)
			
			# get_tok_list(from_tokens, id_to_token)
			# get_tok_list(to_tokens, id_to_token)
			# get_tok_list(labels, id_to_label)

			from_tokens_flat, from_tokens_lens = element_wise_pad_flat(from_tokens, contexts_per_label, token_to_id)
			to_tokens_flat, to_tokens_lens = element_wise_pad_flat(to_tokens, contexts_per_label, token_to_id)
			...
			for ii in range(batch_sz):
				all_toks = from_tokens_flat[ii][:]
				all_toks.extend(to_tokens_flat[ii][:])
				replace_map_i, site_map, status = get_all_replacement_toks(np.array(all_toks), None, token_to_id, replace_tokens)
				# print_flat_tokens(all_toks, id_to_token)
				...
				mask = np.array(len(all_toks)*[False]).astype(bool)
			...
			#	new_inputs, site_map_map, z_all_map, input_lengths, sites_to_fix_map, max_size = replace_toks_batch(start_tokens_flat, start_tokens_lens, filenames, z_map, site_map_map, site_map_lookup_map, best_replacements_batch, field_name, token_to_id, id_to_token, orig_tok_map)
				...
				if u_optim and alt_iters%2 == 0:
					...
					for j in range(u_pgd_epochs):
						...
						if use_u_discrete:
							...
							logits = model(context_map[ii], [contexts_per_label[ii]], labels.shape[0], labels[:, ii].unsqueeze(1), already_one_hot=False)
						else:
							logits = model(context_map[ii], [contexts_per_label[ii]], labels.shape[0], labels[:, ii].unsqueeze(1), already_one_hot=False)
						loss = model._calculate_loss(logits, batch.labels[:,ii].unsqueeze(1))
...
if __name__=="__main__":
	...
	if not opt.no_gradient:
		...
		# for field_name, _ in fields_inp:
		field_name = 'transforms.Replace' # need to fix this
		# if field_name in ['src', 'tgt', 'index', 'transforms.Identity']:
		# 	continue
