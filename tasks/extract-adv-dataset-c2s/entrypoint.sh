...
TEST_NAME="data"
FOLDER=/mnt/inputs/transforms.Replace
IDENTITY_DATA_PATH=/mnt/inputs/transforms.Replace/transforms.Identity
...
if [ "${RUN_SETUP}" == "true" ]; then
    python3 -m data_preprocessing.preprocess_code2seq_data data code2seq --collect-vocabulary --data_folder $FOLDER --just_test --test_name $TEST_NAME
fi
  ...
  for t in $(seq 1 ${NUM_T}); do
    ...
    if [ "${NO_GRADIENT}" != "true" ]; then
      python3 /code2seq/replace_tokens.py \
        --source_data_path /mnt/outputs/test1.tsv \
